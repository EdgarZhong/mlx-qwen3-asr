...
def _encode_single_reference_loop(
    ...
    *,
    ...
    """按Qwen官方pad_sequence语义构造独立逐块基准，不能把旧MLX循环当真值。"""
    total_frames = mel.shape[1]
    # 官方先把本条音频所有块补到最长块，再卷积，最后用有效长度mask裁剪。
    # 不足一整块的短音频只补到自身长度；超过一块时最长块为chunk_size。
...
class TestAudioEncoderEncodeSingleOptimized:
    ...
    def test_matches_reference_loop_with_tail_chunk(self, frames):
        """覆盖短句、整块及不同模8尾长，确保补齐不多出有效音频token。"""
        mx.random.seed(20260918)
        cfg = _tiny_audio_config()
        encoder = AudioEncoder(cfg)
        # 小模型默认bias为0会掩盖尾部补齐差异；真实训练权重包含非零bias。
